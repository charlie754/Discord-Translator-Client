...
**Two different things can send text out of a DM, and only one of them is governed by a setting.**

**Automatic translation** — the whole-channel kind, where the messages you scroll past are sent
for translation as they are rendered — excludes direct messages and group DMs by default. Enable
the `includeDMs` setting and DMs are included in it, and their text is sent to the translation
service. **That setting governs automatic translation, and only that.**

**Selection translation is not governed by it.** A deliberate double-click or triple-click sends
**the text you selected** to the translation service, in any conversation this app can identify:
a server channel whose translator toggle is off, and a direct message or group DM with
`includeDMs` off. It is **per action** rather than ongoing — one gesture, one selection, and
nothing else from that conversation goes with it. It also has to be a click you really made: a
click generated by a script rather than by you is refused before anything is read or sent.

**One gesture can still mean more than one request, for that same selection.** If an attempt
fails in a way that looks temporary — no network, a rate limit, a 5xx from the provider — it is
tried again, up to three more times, with a pause between attempts. Every attempt
sends the **same** selected text and nothing new, so what the provider receives does not grow;
what changes is how many times it receives it.

**Closing a popover retires the gesture, and it retires the whole of the gesture.** A click
anywhere closes the popover that is open, and closing it ends whichever gesture is in progress —
including one that has not put anything on screen of its own yet. It is the popover that carries
this: with nothing on screen there is nothing to close, so a click on its own is not a cancel.

**Before a gesture's request has gone out, closing the popover means nothing is sent.** There are
three points in one gesture where that matters. A double-click pauses for about half a second
first, in case it turns out to be the first half of a triple-click, and for that half second it
has nothing on screen of its own. A gesture can then wait its turn behind other translations. And
a failed attempt pauses again before it is retried. A click landing in any of the three — the
click that closes the previous gesture's popover, for instance — ends that gesture, and its
selection is not sent at all: an attempt that had not yet been made is not made, and a retry that
had not yet been made is not made either.

**A request already on its way is still completed, and the provider still receives and logs it:
nothing in this app can un-send it.** What closing the popover does to that one is discard its
answer, which then paints nothing; before, a retry that succeeded a second later reopened the
popover you had just dismissed. The same holds if you switch the plugin off while an attempt is in
flight.

**So a double-click in a DM does send that selection to the provider even with `includeDMs`
off.** That is deliberate and it is stated here rather than left to be discovered. The request
goes to the same provider as any other translation — Google's public endpoint with the shipped
settings, or your own Apps Script deployment if you configured one — and what each host receives
is exactly what is listed under
[Every Server This App Contacts](#every-server-this-app-contacts).

**There is no setting that turns the gesture off.** If you do not want a selection in a DM sent
for translation, do not double-click or triple-click it.

## Control

**Automatic translation only occurs when you enable it for a specific server with the per-server
toggle**, and it starts off for every server every time the app starts. A DM additionally needs
`includeDMs`.

**Selection translation is controlled by the gesture, not by a setting.** It sends only the text
you selected, only when you double-click or triple-click it, and only in a conversation this app
can identify. If it cannot tell which conversation the text came from — a search result, for
instance — it refuses and says so on screen rather than guessing.
