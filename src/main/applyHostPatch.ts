    ...
    try {
        ...
        for (let i = undo.length - 1; i >= 0; i--) {
            try {
                ...
                console.error("[Discord Translator] Rollback step failed", cleanupErr);
            }
        }
        throw err;
    }
    ...
    try {
        for (const name of readdirSync(discordPath)) {
            ...
            try {
                ...
                console.error("[Discord Translator] Skipping unreadable sibling", name, statErr);
                continue;
            }
            if (!isDir) continue;
        ...
        console.error("[Discord Translator] Failed to scan for sibling versions", err);
        return null;
    }
